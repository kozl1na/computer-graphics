...
namespace Lab3
{
    ...
    {
        int[,] hex = new int[6, 3];      // матрица фигуры
        int[,] hex_temp = new int[6, 3]; // копия
        int[,] matr = new int[3, 3];     // матрица преобразования

        double ugol = 0;       // текущий угол
        double ugol_b = 10; // угол B
        double size = 20;      // начальный размер
        bool f = true;
        ...
        private void Draw_text_logo()
        {
            Graphics g = Graphics.FromHwnd(pictureBox1.Handle);

            Font f = new Font("Arial", 10, FontStyle.Bold);

            g.DrawString("62 года ТУСУР", f, Brushes.DarkBlue, 20, 20);

            g.DrawImage(logo, pictureBox1.Width - 150, 20, 110, 110);
